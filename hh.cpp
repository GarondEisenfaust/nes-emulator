...
unsigned TestCard(int x, int y) {
  ...
  int c =
      (xgridsub == 0 || ygridsub == 0)
          ? white                                                      // white grid
          : ((ygrid < 0 || ygrid >= 14) ? (xgrid & 1) ? white : black  // white or black checkerboard at top & bottom
                                        : 0x00);                       // gray otherwise
  ...
  if (y > 24 && ygrid < 13) {
    ...
    if (xgrid >= 0 && xgrid <= 1 && (xgrid == 0 ? xgridsub > 0 : nook)) c = lobe[(y < ycenter) + 2 * (xgrid == 1)];

    if (xgrid >= 13 && xgrid <= 14 && (xgrid == 14 ? xgridsub > 0 || nook : (nook && xgridsub > 0)))
  ...
  switch (ygrid) {
    case 1:
      ...
      break;
    case 2:
      if (xdist > width * 0.18 || x == (int)(width * 0.35)) c = black;
      break;
    case 11:
      if (xdist <= width * 0.18 && x != (int)(width * 0.35))
        c = 0x0F + 0x10 * ((x >> 3) & 3);  // Use four different blacks for the station ID background
      break;
    case 3:
      c = (x + 5) % 30 < 15 ? 0x10 : black;
      break;  // gray or black
    case 4:
    case 5:
    case 6: {
...
class PNGencoder {
 ...
 private:
  ...
  static unsigned adler32(const unsigned char* data, unsigned size, unsigned res = 1) {
    for (unsigned s1, a = 0; a < size; ++a)
      s1 = ((res & 0xFFFF) + (data[a] & 0xFF)) % 65521, res = s1 + ((((res >> 16) + s1) % 65521) << 16);
    return res;
  }
  static unsigned crc32(const unsigned char* data, unsigned size, unsigned res = 0) {
    for (unsigned tmp, n, a = 0; a < size; ++a, res = ~((~res >> 8) ^ tmp))
      for (tmp = (~res ^ data[a]) & 0xFF, n = 0; n < 8; ++n) tmp = (tmp >> 1) ^ ((tmp & 1) ? 0xEDB88320u : 0);
    return res;
  }
 ...
 public:
  void EncodeImage(unsigned width, unsigned height, const unsigned char* rgbdata) {
    std::size_t o;
#define BeginChunk(n_reserve_extra) \
  o = output.size();                \
  output.resize(o + 8 + n_reserve_extra)
#define EndChunk(type)                                                       \
  do {                                                                       \
...
const float Voltages[4][2] = {{0.350f, 1.090f}, {0.518f, 1.500f}, {0.962f, 1.960f}, {1.550f, 1.960f}};
}  // namespace
