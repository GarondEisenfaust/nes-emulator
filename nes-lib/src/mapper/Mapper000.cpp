...
Mapper000::Mapper000(uint8_t programBanks, uint8_t characterBanks) : IMapper(programBanks, characterBanks) {}

uint32_t Mapper000::CpuMapRead(uint16_t address) { return address & (mProgramBanks > 1 ? 0x7FFF : 0x3FFF); }
uint32_t Mapper000::CpuMapWrite(uint16_t address, uint8_t data) { return CpuMapRead(address); }
